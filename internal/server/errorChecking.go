...
func checkHeaderErrors(hdr *DNSHeader) error {
	...
	if opcode == 0 && hdr.qdcount != 1 {
		...
		return fmt.Errorf("invalid query count")
	}
	...
	if opcode == 1 {
		...
		return fmt.Errorf("obsoleted query no longer suppored")
	}
	...
	if opcode == 2 {
		...
		return fmt.Errorf("server status request not supported")
	}
	...
	if ((opcode < 5) && (opcode > 2)) || (opcode > 6) {
		...
		return fmt.Errorf("reserved opcode")
	}
	...
	if opcode == 5 {
		...
		return fmt.Errorf("update query not supported")
	}
